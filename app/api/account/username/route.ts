...
import { renameUsername, UsernameError } from "@/lib/services/username"
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    await renameUsername(session.userId, parsed.data.username)
    return NextResponse.json({ data: { username: parsed.data.username } })
  } catch (err) {
    if (err instanceof UsernameError) {
      const status = err.code === "USERNAME_TAKEN" || err.code === "USERNAME_RESERVED" ? 409 : 400
