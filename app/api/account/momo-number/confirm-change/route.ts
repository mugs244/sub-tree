...
const schema = z.object({
  new_momo_number: z.string().regex(ugandaPhoneRegex),
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    await confirmNumberChange(session.userId, "momo_number", parsed.data.new_momo_number, parsed.data.code)
  } catch (err) {
  ...
  return NextResponse.json({ data: { momo_number: parsed.data.new_momo_number } })
}
