...
const schema = z.object({
  new_momo_number: z.string().regex(ugandaPhoneRegex, "Enter a valid Uganda mobile number (e.g. 0771234567)"),
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    await requestNumberChange(session.userId, parsed.data.new_momo_number, parsed.data.channel)
  } catch (err) {
